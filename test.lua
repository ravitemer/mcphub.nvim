...
local function workspace_symbol_hover(query)
  ...
  promise.resolve, promise.reject = function(value) fulfill(value) end, function(err) reject(err) end
...
handler = function(req, res)
	...
	local request_success, request_error = pcall(function()
		...
		if timed_out or response_sent then return end
		...
		local hover_success, hover_error = pcall(function()
			...
			if not hover_result or not hover_result.contents then
			    ...
			    local fname = vim.uri_to_fname(uri)
			    send_response(function()
				res:text("Symbol found at " .. fname .. ":" .. (range.start.line + 1) .. " (no documentation available)"):send()
			    end)
			    return
			end
